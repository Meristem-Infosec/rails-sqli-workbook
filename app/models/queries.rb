Queries = [
  {
    ...
    :explanation => <<-MD
    This example allows you to finds the age of a specific user, rather than the sum of order amounts.
      MD
  },
...

  {
    ...
    :explanation => <<-MD
      This example would likely be found in a login-type flow and gives you an opportunity to either find a specific user by name, or return the first user in the database'
    MD
  },
...

  {
    ...
    :explanation => <<-MD
      The intent of this query is to group non-admin users by the specified column. Instead, the query returns all users. PROBLEM, the column referenced by group_by must exist in the select clause per SQL syntax.  If both are dynamic, then the injection would be into the select clause not the group clause
    MD
  },

  {
    ...
    :sql => 'REPLACE',
    :explanation => <<-MD 
      "This input injects a union in order to return all orders, instead of just the orders from a single user. PROBLEM, the column referenced by group_by must exist in the select clause per SQL syntax.  If both are dynamic, then the injection would be into the select clause not the group clause. The `having` method does not escape its input and is easy to use for SQL injection since it tends to be at the end of a query."
    MD
  },
...

  {
    ...
    :sql => 'REPLACE',
    :explanation => <<-MD
      Skip WHERE clause and return all orders instead of just the orders for the specified user. The `joins` method can take an array of associations or straight SQL strings. Again, when would the table name for a join be user controlled input.
    MD
  },
